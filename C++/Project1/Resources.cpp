...
{
    string monthErrorMsg = "invalid month value in setDate\n";
    string dayErrorMsg = "invalid day value in setDate\n";
    string yearErrorMsg = "invalid year value in setDate\n";

    string finalErrorMsg = "";
    std::ostringstream errorStrm;
    errorStrm <<"invalid values in setDate("<< mm << ", " << dd <<", " << yyyy << ")\n";
    bool allOK= true;
    
    
    // test for valid values before updating data members
    
    if ( mm < MIN_MM || mm > MAX_MM )
    {
        // add code to throw an exception in addition to the output
        // cout << monthErrorMsg << endl;
        // throw std::invalid_argument(monthErrorMsg);
        allOK = false;
        errorStrm << "month is not valid\n";

    }
    
    if ( dd < MIN_DD || dd > MAX_DD[mm] )
    {
        // add code to throw an exception in addition to the output
        // cout << dayErrorMsg << endl;
        // throw std::invalid_argument(dayErrorMsg);
        allOK = false;
        errorStrm << "day is not valid\n";

    }
    
    if ( yyyy < MIN_YYYY || yyyy > MAX_YYYY )
    {
        // add code to throw an exception in addition to the output
        // cout << yearErrorMsg << endl;
        // throw std::invalid_argument(yearErrorMsg);
        allOK = false;
        errorStrm << "year is not valid\n";

    }

    finalErrorMsg = errorStrm.str();
    if (!allOK){ throw std::invalid_argument(finalErrorMsg);}
...
/****************************************************************************
 *                  Date class member function getDate                      *
 ****************************************************************************/
...
/****************************************************************************
 *                     Date class assignment operator                       *
 ****************************************************************************/
...
/****************************************************************************
 *                 Time class overloaded operators                          *
 ****************************************************************************/
...
bool Date::operator==( const Date &dt) const
{
    ...
    answer = this->mm == dt.mm && this->dd == dt.dd && this->yyyy == dt.yyyy;
